...
public class Car3 {
    ...
    // 숫자를 사용하기 좋게 변수로 정의할 때도 static 변수로 만든다.
    // 이렇게 특정 값을 변수로 정해두는 것을 상수라고 한다.
    // 보통 상수는 대문자로 정의한다.왜? 일반 변수와 쉽게 구분하기위해
    // 특히 상수는 조회용이기 때문에 final을 붙여 값을 변경할수 없게 한다.
    // 다른 패키지에 있는 클래스에서도 사용할 수 있도록  public 을 붙여 완전 공개한다.
