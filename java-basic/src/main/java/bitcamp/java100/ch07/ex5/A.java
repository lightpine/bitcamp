...
public class A {
    ...
    /*
     A(){
        ...
        // 모든 생성자는 첫번째 문장으로 수퍼클래스의 기본 생성자를 호출한다.
        // A클래스의 수퍼 클래스는 OBJ이기 때문에 
        // 다음은 OBJ의 기본 생성자를 호출한다.
        super();
    ...
    */
    ...
    A(){
        // 수퍼 클래스의 기본 생성자를 호출하는 문장을 생략하게되면 컴파일러가 자동으로 첫번째 문장으로 다음을 추가한다.
    ...
    // 때로는 다음과 같이 생성자를 정의 할 때 수퍼 생성자 호출을 생략하지 않을 수 있다.
    A(String str){
        super();// 이렇게 수퍼 클래스의 생성자를 호출하라고 명시할 수 있다.
        System.out.println("A(String) 호출됨");
    }
    
    // 그런데 수퍼 클래스 생성자를 첫번째로 후출하지 않는다면 컴파일 오류!
    A(int i){
        //super();// 생략하면 자동으로 붙는다.
