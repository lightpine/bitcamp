// 캡슐화의 존재 이유
// 인스턴스 변수에 무효한 값이 저장된는 것을 방지함으로써
// 추상화가 무너지는 것을 막는다.
...
// 어떻게? 객체지향 분석/설계(OOA/D, Object-Oriented Analysis and Design)를 통해
...
public class Test7_1 {
    public static void main(String[] args) {
        ...
        // 이것은 괴물 데이터이다.
        // 이렇게 멤버 객체에 Gym회원에 맞지 않는 무효한 괴물 데이터를 넣게되면
        // 시스템 실행시 원하지 않는 데이터를 뽑게 된다.
        // 예) Gym 회원의 평균 몸무개와 평균키를 뽑을 때
        // 잘못된 데이터 때문에 전체 회원의 평균 몸무게와 평균 키 값이 왜곡되는 문제가 발생한다.
        ...
        // 환자의 생명이 위독할 수 있는 경웅가 발생할 것이다.
        
        // 그럼 이렇게 무효한 값이 들어가는 문제가 발생하는 이유는 뭔가?
        // 개발자가 직접 변수에 접근해서 임의의 값을 넣을 수 있기 떄문이다.
        // 이것을 어떻게 막아야 하는가?
        // 개발자가 직접 변수에 접근하지 못하게 막아야 한다.
        // 또는 사용 권한이 없는 코드에서 이 변수의 값을 임의적으로 변경하는 것을
        // 막아야 한다.
        
        // 방법은?
        // 변수나 메서드에 접근 범위를 조정하면 된다.
        // 모든것을 완벽하게 막지는 못하겠지만 그래도 잘못된 무효한 값을
        // 넣을 가능성을 최소화 시키는 것이 좋다!
        m.weight = 900.7f;
        m.height = 16.0f;
    }
